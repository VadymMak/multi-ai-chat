...
const ProjectSelector: React.FC = () => {
  ...
  const [loading, setLoading] = useState(false); // UI spinner only; never disables the <select>
  ...
  const loadProjects = useCallback(async () => {
    ...
    try {
      ...
      if (!projectId && sanitized.length > 0) {
        ...
        // Fire and forget; internal guards prevent double-init
        runSessionFlow(roleId, firstId, "ProjectSelector[Auto]").catch(
          () => {}
        );
      }
    } catch (err) {
  ...
  const handleChange = useCallback(
    async (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      try {
        ...
        // ALWAYS clear loading so the dropdown never gets stuck
        setLoading(false);
      }
    },
