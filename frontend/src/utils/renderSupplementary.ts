...
export async function renderSupplementary(
  ...
  try {
    ...
    // 3) explicitly mark that bubble as done
    const store = useChatStore.getState();
    // try common patch methods if your store has them
    (store as any).setMessageTyping?.(id, false) ||
      (store as any).patchMessage?.(id, { isTyping: false }) ||
      (store as any).updateMessage?.(id, (m: ChatMessage) => ({
        ...m,
        isTyping: false,
      }));
    // if none of these exist, we’ll add one tiny helper in chatStore tomorrow.
  }
}
