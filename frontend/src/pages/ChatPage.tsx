...
const ChatPage: React.FC = () => {
  ...
  const handleSend = useCallback(
    async (
      ...
      let messageToSend = clean; // ИЗМЕНЕНО на let вместо const
        ...
        for (const file of attachments) {
          try {
            ...
            if (result) {
              ...
              // Собираем контекст из загруженного файла
              const fileContext = `
[File: ${file.name}]
...
${result.summary ? `Summary: ${result.summary}` : "No content extracted"}
        ...
        // ВАЖНО: Добавляем контекст файлов к сообщению для AI
        if (fileContexts.length > 0) {
          ...
          console.log("📤 Message with file context:", messageToSend);
        }
      }

      // PDF command mode (оставляем как есть)
      const pdfLines = parsePdfCommand(messageToSend);
      if (pdfLines && pdfLines.length) {
        // ... существующий PDF код без изменений ...
        setTyping(true);
        abortRef.current?.abort();
      ...
      // Add user bubble - показываем оригинальный текст без контекста файлов
      const userMessage = {
        id: `user-${uuidv4()}`,
        sender: "user" as Sender,
        text: clean, // ВАЖНО: показываем clean, а не messageToSend
        role_id: roleId,
        project_id: projectId,
        chat_session_id: sid,
      };
      ...
      try {
        if (provider === "boost") {
          ...
          // YouTube/Web обработка остается без изменений
          ...
          // ... остальной код обработки ответа без изменений ...
