...
const ChatPage: React.FC = () => {
  ...
  // Observe InputBar wrapper height
  useEffect(() => {
  ...
  // NEW: bootstrap or restore chat session on mount / role+project availability
  useEffect(() => {
    if (!hasHydrated) return;
    const store = useChatStore.getState();

    // Avoid duplicate work if already ready
    if (store.sessionReady) return;

    // If we have a persisted marker, try to restore first
    ...
    // Otherwise, when role/project are selected, load or init a session
    if (roleId && projectId) {
      store.loadOrInitSessionForRoleProject(roleId, Number(projectId));
    }
  ...
  const handleSend = useCallback(
    async (text?: string) => {
      ...
      const { clean, kind } = parseRenderOverride(raw);
      ...
      try {
        if (provider === "boost") {
          ...
          const result = await sendAiMessage(
            ...
            projectId,
            sid,
            // pass render override if present (ChatPage → aiApi maps markdown→doc)
            kind ? { kind } : undefined
          );
          ...
          // NEW: pull render meta from /ask result and attach to placeholder
          const renderMeta =
  ...
  const handleFileDrop = useCallback(
    async (file: File): Promise<string | null> => {
      ...
      try {
        const result = await uploadFile(file, roleId, projectId, sid);
        if (result?.summary) {
    ...
    [handleSend, roleId, projectId, chatSessionId, ensureSessionId]
  );
