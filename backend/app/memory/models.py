...
class Role(Base):
    ...
    canon_items = relationship("CanonItem", back_populates="role", cascade="all, delete")
...
class Project(Base):
    ...
    # Relationships (view-only mirrors for FK-int join convenience)
...
class MemoryEntry(Base):
    ...
    # NOTE: inserting with a non-existent role_id will raise FK error — create roles first.
...
class CanonItem(Base):
    ...
    tags = Column(JSON, nullable=True)   # SQLite stores as TEXT; SQLA handles it
    ...
    # Relationships
    role = relationship("Role", back_populates="canon_items")
    project = relationship(
        "Project",
