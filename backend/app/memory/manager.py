...
# Late import so tiktoken import errors don't break module load
...
class MemoryManager:
    ...
    def preflight_token_budget(
        ...
        total = 0
        for t in texts or []:
            total += self.count_tokens(t or "")
        near_soft = total >= soft
    ...
    def count_tokens(self, text: str) -> int:
        ...
        try:
            if _enc is not None:
                ...
                tokens = _count_tokens_fallback(text)
    ...
    def store_chat_message(
        ...
        content = safe_text(content)[:MAX_RAW_TEXT_LEN]
        sender = safe_text(sender) or "assistant"
        tokens = self.count_tokens(content)

        raw_text = f"{sender}: {content}"
        preview = content.strip()[:MAX_SUMMARY_LEN]
    ...
    def insert_audit_log(
        ...
        try:
            # Local import prevents early NameError on reloads
