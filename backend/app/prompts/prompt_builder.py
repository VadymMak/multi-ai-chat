...
from app.memory.utils import get_project_structure  # ✅ NEW
...
def build_full_prompt(
    ...
    # ✅ Local token counter using MemoryManager
    memory = MemoryManager(db=None)  # db not needed for token counting only
    ...
    # ✅ Safe fallback for system prompt
    system_prompt = system_prompt.strip() or "You are a helpful assistant."
    project_structure = project_structure.strip() if project_structure else "No project structure provided."
...
def generate_prompt_from_db(
    ...
    # ✅ Handle missing or invalid project_id
