...
from app.providers.youtube_provide import search_youtube
...
# ---------- Output-mode helpers (safer detection) ----------
_LANG_HINTS = {
    "c#": "csharp",
    "csharp": "csharp",
    "python": "python",
    ...
    # NOTE: "go" is intentionally handled as risky unless explicitly asking for code
    "go": "go",
}
...
def _detect_output_mode(
    ...
    """
    Return ('code'|'doc'|'plain', normalized_lang_or_None).
    Safer detection to avoid false positives (e.g., 'Pushkin' contains 'sh').
    """
    ...
    # If user already provided a fenced block, treat as code
    if "```" in q:
    ...
    # Explicit code intent in wording
    ...
    # Whole-word language detection (ignore risky short aliases unless explicit intent)
...
def _enforce_code_block(text: str, lang: Optional[str]) -> str:
    """Extract first fenced block; otherwise wrap everything as fenced code with normalized lang."""
    text = text or ""
...
def _normalize_bullets_to_markdown(text: str) -> str:
    if not text:
        return text
...
def _suggest_filename(lang: Optional[str]) -> Optional[str]:
    ...
    return {
        ...
        "text": "poem.txt",   # used for poem_code
    }.get(l)
...
async def _safe_youtube(topic: str) -> List[Dict[str, str]]:
    ...
    try:
        items = await asyncio.wait_for(asyncio.to_thread(search_youtube, topic, maxn), timeout=timeout)
...
def _format_youtube_for_prompt(items: List[Dict[str, str]]) -> List[str]:
    if not items:
        return ["None."]
...
class AskRequest(BaseModel):
    ...
    # Presentation modes used by UI toggles (poem workflows etc.)
    ...
    def project_id_safe(cls, v):
        ...
        return s  # store as string for MemoryManager APIs
...
async def ask_route(data: AskRequest, db: Session = Depends(get_db)):
    # ---- Validate FK existence up front
    ...
    try:
        ...
        if hasattr(memory, "preflight_token_budget"):
            budget = memory.preflight_token_budget([m["content"] for m in hist_now])  # type: ignore[attr-defined]
        else:
            ...
            budget = {
                ...
                "soft": soft,
                "hard": hard,
                "near_soft": total >= int(0.9 * soft),
                "over_hard": total >= hard,
            }
        try:
            memory.insert_audit_log(
                project_id, role_id, chat_session_id, "internal", "token_preflight",
                f"total={budget['total_tokens']} soft={budget['soft']} hard={budget['hard']}"
            )
        except Exception:
            pass

        if budget["over_hard"] or budget["near_soft"]:
        ...
        if bool(getattr(settings, "ENABLE_YT_IN_CHAT", True)):
            ...
            if yt_topic:
                ...
                if youtube_items:
                    youtube_context_blocks = _format_youtube_for_prompt(youtube_items)
                    try:
                        memory.insert_audit_log(
        ...
        # Force plain whenever we detected YT intent OR explicit poem_plain
        force_plain = (yt_topic is not None) or (data.presentation == "poem_plain")
        # Force single code block when poem_code
        ...
        # 5) Trim history for the model, ensure the latest user turn is included
        ...
        def _post_process(answer: str) -> Tuple[str, Optional[Dict[str, Any]]]:
            ...
            if force_code:
                # ensure exactly one code block labeled `text`
            ...
            if force_plain:
                # strip any fenced code that might slip through
