...
_LANG_HINTS = {
    ...
    "go": "go",  # risky short alias if no explicit code intent
}
...
def _detect_output_mode(
    ...
    """
    Return ('code'|'doc'|'plain', normalized_lang_or_None) with safer detection
    (avoid false positives like 'sh' inside 'Pushkin').
    """
    ...
    # Whole-word language detection (skip risky short aliases without explicit intent)
...
def _enforce_code_block(text: str, lang: Optional[str]) -> str:
    """Extract first fenced block; else wrap the body in a single fenced block."""
    text = text or ""
...
async def ask_ai_to_ai_route(data: AiToAiRequest, db: Session = Depends(get_db)):
    ...
    # ---- Context gathering (safe, non-blocking) ----
    try:
        yt_block, yt_hits = await run_in_threadpool(perform_youtube_search, data.topic, memory, role_id, project_id)
    except Exception as e:
        yt_block, yt_hits = "[YouTube context unavailable]", []
        print(f"[YouTube Error] {e}")

    try:
        web_hits = await run_in_threadpool(perform_web_search, data.topic)
    except Exception as e:
        web_hits = []
    ...
    # -------------------- Render policy (same spirit as /api/ask) --------------------
    base_system = (
    ...
    # We’ll inject policy as a system message for the starter and review steps,
    # AND append it to the final system prompt for the summary step.
    ...
    web_context = [f"{v['title']} - {v.get('snippet','')}" for v in web_hits]
