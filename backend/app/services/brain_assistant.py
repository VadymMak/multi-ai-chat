...
def _parse_reminder_sync(directive: str, tz: str = "Europe/Bratislava") -> dict:
    ...
    system = (
        ...
        '"fire_at": ISO 8601 datetime в локальном времени пользователя, без суффикса timezone, '
        "должен быть в будущем.\n"
        '"text": ТОЛЬКО ПРЕДМЕТ напоминания (о чём напомнить) на языке пользователя, кратко. '
        "НЕ включай слово-триггер (напомни, remind), "
        "НЕ включай временное выражение (через 2 минуты, в 9 утра, завтра и т.п.) — "
        'только суть. Если предмет не указан — верни "Напоминание".\n'
