...
) or os.getenv("OPENAI_DEFAULT_MODEL", "gpt-4o")
...
# Enable JSON mode by default if settings say so; otherwise ENV flag
JSON_MODE_DEFAULT = (
...
def _should_use_max_completion_tokens(model_name: str) -> bool:
    """
    Reasoning/GPT-5 families require `max_completion_tokens`.
...
def _extract_text_from_choice(choice: Any) -> str:
    ...
    try:
        ...
        # 1) Normal content (string or list-of-parts)
        ...
        # 3) tool_calls (modern function calling)
...
def _build_messages(msgs: List[dict], sys_prompt: Optional[str], *, force_text_only: bool) -> List[dict]:
    ...
    if FORCE_TEXT_ONLY or force_text_only:
        # Nudge ALL calls to return plain text
        extra = " Always respond in plain text. Do not call tools/functions. Do not return JSON unless explicitly requested."
...
def _call_chat_create(
    ...
    *,
    ...
    # If .with_options() worked, timeout is set on client2; we also pass here for older SDKs.
    ...
    *,
    ...
    try:
        ...
        if not resp or not getattr(resp, "choices", None):
            ...
            # Fall back immediately on no choices
        ...
        if not text:
            print("⚠️ [OpenAI Warning] No text content in any choice; trying fallback parse/redo")
            # Try a simplified second pass on fallback model (force text, JSON off)
            return _fallback_plain_text(client, fallback_messages=final_messages)
        ...
        try:
            ...
            if usage:
                ...
                print(
                    f"✅ [OpenAI] model={model} out≤{max_tokens} usage total={total} prompt={prompt_toks} completion={comp_toks}"
                )
            else:
...
def _fallback_plain_text(client: Any, *, fallback_messages: List[dict]) -> str:
    ...
    try:
        ...
        # Add a gentle instruction without overwriting the user content
        ...
        if has_system:
            ...
            fb_msgs = [{"role": "system", "content": "You are a helpful assistant. Return a concise plain-text answer. Do not call tools/functions."}] + fb_msgs
