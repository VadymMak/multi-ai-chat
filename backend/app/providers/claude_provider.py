...
def ask_claude(
    ...
    # Step 1: Clean and prepare message history
    if memory is None:
    ...
    # Step 2: Convert to Claude's expected format
    final_messages: List[MessageParam] = []
    for msg in full_history:
        role = msg.get("role")
        content = msg.get("content", "").strip()
        if role not in {"user", "assistant"} or not content:
            print(f"⚠️ [Claude] Skipping invalid message: {msg}")
            continue
    ...
    # Step 3: Retry loop for stability
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            print("🟡 [Claude] Sending message payload:")
            print(json.dumps(kwargs, indent=2)[:600] + "...")
            ...
            if not isinstance(first_block, TextBlock) or not first_block.text.strip():
                raise AnthropicError("Claude returned empty or invalid response block")

            answer = first_block.text.strip()
            print(f"✅ [Claude] Success: model={model}, tokens≈1024")
            return answer

        except AnthropicError as ae:
            if "overloaded" in str(ae).lower() and attempt < MAX_RETRIES:
                print(f"🔁 [Claude] Overloaded, retrying ({attempt}/{MAX_RETRIES})...")
